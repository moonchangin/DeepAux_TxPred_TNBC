...
class BreastCancerDataset(Dataset):
    ...
    def __init__(self, file_paths):
        ...
        # Select top 500 genes from the training dataset
        # self.top_genes = self.select_k_best_genes(file_paths[0])

        # Load and normalize the data using the selected genes
        data, labels = self.preprocess_data(file_paths, self.top_genes)
    ...
    def select_k_best_genes(self, file_path):
        """
        Select the top 500 genes based on univariate feature selection.

        Args:
            file_path (str): The file path containing the training dataset.

        Returns:
            list: A list of the top 500 genes.
        """
        ...
        selector.fit(features, actual_labels)

        # Get the top 500 genes
        top_genes = selector.get_support(indices=True)
        gene_names = df.columns[4:][top_genes]

        return gene_names
    
    def preprocess_data(self, file_paths, top_genes):
        """
        Preprocess the data using the selected top genes.

        Args:
            file_paths (list): A list of file paths containing the datasets.
            top_genes (list): A list of the top selected genes.

        Returns:
            DataFrame: A DataFrame containing the preprocessed features.
            ndarray: An array containing the labels.
        """
        ...
        for file_idx, file_path in enumerate(file_paths):
            ...
            actual_labels = df.iloc[:, 1].values
            # Select the genes that are present in the current dataset
            # available_genes = [gene for gene in top_genes if gene in df.columns]
            # features = df[available_genes]
